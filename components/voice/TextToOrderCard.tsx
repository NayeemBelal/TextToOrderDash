  ...
  return (
    <div className="bg-capy-card rounded-2xl border border-capy-border flex flex-col overflow-hidden">
      ...
      <div className="px-5 py-4 flex items-center justify-between">
        ...
        >
          <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-capy-card shadow-sm transition-transform duration-200 ${enabled ? 'translate-x-5' : 'translate-x-0'}`} />
        </button>
      </div>
      <div className="px-5 pb-4">
        {error && <p className="text-red-500 text-xs mb-1">{error}</p>}
        <div className={`px-3 py-2 rounded-lg text-xs font-semibold ${enabled ? 'bg-capy-green-light text-capy-green-dark' : 'bg-capy-surface text-capy-muted'}`}>
